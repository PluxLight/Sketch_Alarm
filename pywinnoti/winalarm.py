...
class WinAlarm():
    ...
    def __init__(self):
        ...
        self.pixiv_cookie = {'PHPSESSID' : ''} #Cookie.json에서 값을 읽어온다
    ...
    def read_coookie(self):
        ...
        try:
            with open('Cookie.json', 'r') as file:
                self.pixiv_cookie = json.load(file)

        except Exception as e:
            print(e + '\n')
            print('Cookie.json 파일을 읽는데 실패했습니다.')
            self.end_program()
    ...
    def crawl(self):
        ...
        if len(self.artists) == 0:
                ...
                try:
                    ...
                    if req.status_code == 200:
                        ...
                        if 'Live broadcasting' in html and self.artists_status[self.artist] == 'False':
                            ...
                            print(f'\n{self.nick}({self.artist})님이 방송중입니다.') #콘솔에서 보일거
                        ...
                        print("유저 정보 확인에 실패했습니다. user.txt에 입력한 유저값이 정상인지 확인하세요.") #콘솔에서 보일거
                        self.end_program()
            ...
            print(f'\n{self.crawl_count}번째 크롤링이 끝났습니다.\n1분 후 작업을 다시 시작합니다.')
            self.crawl_count += 1
